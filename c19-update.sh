...
# If no flag is specified default to USA
if [[ -z "$COUNTRY" ]]; then
    COUNTRY="USA"
fi
...
case "$response" in
[yY][eE][sS]|[yY])
curl https://corona.lmao.ninja/v2/countries/$COUNTRY > ~/.cache/corona-usa ;;
*) ;;
esac
